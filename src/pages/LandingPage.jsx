...
const features = [
  ...
  [Sparkles, 'bg-paper text-ink ring-1 ring-black/10', 'text-faith', 'A clear “happening now” view', 'Live progress and the next block help you focus on the present without losing the bigger picture.'],
  ...
  [MapPin, 'bg-paper text-ink ring-1 ring-black/10', 'text-faith', 'Optional prayer times', 'Use location-aware prayer times when they are useful, or keep planning without them.'],
...
function ProductMockup() {
  ...
  return <div className="relative mx-auto w-full max-w-[620px]" aria-label="Preview of the Daycraft routine dashboard">
    ...
    <div className="overflow-hidden rounded-[28px] bg-paper p-3 shadow-[0_28px_70px_-30px_rgba(26,26,26,.45)] ring-1 ring-black/10 sm:p-5">
      <div className="flex items-center justify-between border-b border-black/[.07] px-1 pb-3"><div><p className="font-display text-2xl font-bold leading-none sm:text-3xl">Today</p><p className="mt-1 font-sans text-[10px] font-semibold uppercase tracking-[.16em] text-ink/45">My weekday rhythm</p></div><span className="rounded-full bg-career/10 px-3 py-1 font-sans text-[10px] font-bold uppercase text-career">On track</span></div>
      <div className="mt-4 grid gap-3 sm:grid-cols-[.82fr_1.18fr]">
        <div className="rounded-[20px] bg-cream p-4 ring-1 ring-black/[.06]">
          <div className="flex items-center justify-between gap-3"><p className="font-display text-2xl font-bold">Daily checklist</p><span className="font-sans text-[10px] font-bold text-career">2 of 3</span></div>
          <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-black/10"><div className="h-full w-2/3 rounded-full bg-career" /></div>
          <ul className="mt-3 space-y-2">{habits.map(([label, done, color]) => <li key={label} className={`flex min-h-10 items-center gap-2 rounded-xl bg-white px-3 font-sans text-[11px] font-semibold ring-1 ring-black/[.05] ${done ? 'text-ink/45' : 'text-ink'}`}><span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-md border-2" style={{ borderColor: color, backgroundColor: done ? color : 'transparent' }}>{done && <Check className="h-3 w-3 text-white" strokeWidth={3} />}</span><span className={done ? 'line-through' : ''}>{label}</span></li>)}</ul>
        </div>
        <div className="space-y-2.5">
          <div className="relative overflow-hidden rounded-[20px] bg-ink p-4 text-white shadow-[0_0_0_2px_rgba(15,118,110,.75),0_16px_30px_-18px_rgba(15,118,110,.9)]"><span className="absolute inset-y-0 left-0 w-1.5 bg-career" /><div className="flex items-center justify-between gap-2"><span className="font-sans text-[10px] font-bold uppercase tracking-[.14em] text-white/65">9:00 AM – 11:00 AM</span><span className="flex items-center gap-1 rounded-full bg-career px-2 py-0.5 font-sans text-[9px] font-bold uppercase"><span className="h-1.5 w-1.5 rounded-full bg-white" /> Now</span></div><p className="mt-2 font-sans text-sm font-bold sm:text-base">Focused project work</p><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/15"><div className="h-full w-[58%] rounded-full bg-career" /></div></div>
          <div className="relative overflow-hidden rounded-[18px] bg-white p-3.5 ring-1 ring-black/[.08]"><span className="absolute inset-y-0 left-0 w-1 bg-faith" /><span className="font-sans text-[9px] font-bold uppercase tracking-[.13em] text-ink/45">11:00 AM – 12:00 PM · Next</span><p className="mt-1 font-sans text-xs font-bold">Lunch and reset</p></div>
          <div className="relative overflow-hidden rounded-[18px] bg-taupe p-3.5"><span className="absolute inset-y-0 left-0 w-1 bg-rest" /><span className="font-sans text-[9px] font-bold uppercase tracking-[.13em] text-ink/55">1:00 PM – 2:00 PM</span><p className="mt-1 font-sans text-xs font-bold">Learn something useful</p></div>
        </div>
      </div>
    </div>
  </div>
