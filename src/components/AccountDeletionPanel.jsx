    ...
    <section className="mt-8 rounded-card border border-language/25 bg-language/[.035] p-5 sm:p-6" aria-labelledby="danger-zone-title">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div><p className="font-sans text-[10px] font-bold uppercase tracking-[.18em] text-language">Permanent actions</p><h2 id="danger-zone-title" className="mt-1 font-sans text-lg font-bold text-ink">Danger Zone</h2><p className="mt-1 font-sans text-sm leading-relaxed text-ink/55">Delete this account and everything connected to it.</p></div>
        </div>
      ...
      {!open ? <button type="button" onClick={() => setOpen(true)} className="mt-5 inline-flex min-h-[44px] items-center gap-2 rounded-xl px-4 font-sans text-sm font-bold text-language ring-1 ring-language/35 hover:bg-language/10"><Trash2 className="h-4 w-4" aria-hidden="true" />Delete my account</button> : (
        <div className="mt-5 rounded-2xl bg-white p-4 ring-1 ring-language/20">
          ...
          <input id="delete-confirmation" value={text} onChange={(event) => setText(event.target.value)} autoComplete="off" placeholder="Type DELETE" className="mt-2 min-h-[46px] w-full rounded-xl bg-cream/50 px-4 font-sans text-sm text-ink ring-1 ring-black/15 focus:outline-none focus-visible:ring-2 focus-visible:ring-language" />
          {error && <p role="alert" className="mt-2 font-sans text-xs font-semibold text-language">{error}</p>}
          <div className="mt-4 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" disabled={loading} onClick={close} className="min-h-[44px] rounded-xl px-4 font-sans text-sm font-semibold ring-1 ring-black/15">Keep my account</button><button type="button" disabled={loading || text !== 'DELETE'} onClick={remove} className="flex min-h-[44px] items-center justify-center gap-2 rounded-xl bg-language px-4 font-sans text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-45">{loading ? <Loader2 className="h-4 w-4 motion-safe:animate-spin" aria-hidden="true" /> : <Trash2 className="h-4 w-4" aria-hidden="true" />}{loading ? 'Deleting…' : 'Permanently delete account'}</button></div>
        </div>
      )}
    </section>
  )
